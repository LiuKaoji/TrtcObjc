...
- (void) readVideoFrameFromTime:(float)startTime
                   ...
                   readFinished:(readFinished)readFinished
{
    __block float startTimef = startTime;
    __weak __typeof(self) weakSelf = self;
    dispatch_async(_videoReadedQueue, ^{
        __typeof(self) strongSelf = weakSelf;
        if (!strongSelf)
        ...
        while(sample != nil && CMSampleBufferIsValid(sample)) {
            ...
            if (pts >= startTime && pts <= endTime){
                ...
                if (self->videoSendLastTime / 1000 > currentTime1 - strongSelf.currentTime) {
                    //NSLog(@"video sleep = %ld", videoSendLastTime - currentTime1 * 1000 + strongSelf.currentTime * 1000);
                    usleep(strongSelf->videoSendLastTime - currentTime1 * 1000 + strongSelf.currentTime * 1000);
                }
                readOneFrame(sample);
                CFRelease(sample);
                sample = nil;
                self->videoSendLastTime += (1000 / fps * 1000);
            }else{
                CFRelease(sample);
                sample = nil;
            }
